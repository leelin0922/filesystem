...
<div class="""slidecontainer">
  	<input type="range" 
		...
		max="100" 
		value="<?php 
				$command = "cat /sys/class/backlight/pwm-backlight.0/brightness";
				$readbrightness= (int)exec ($command); 
				$brightness = round($readbrightness *100/236);
				echo $brightness;
				?>" 
...
<?php
	...
	$oldbrightness = round($readbrightness *100/236);
	...
	if($oldbrightness != $newbrightness)
	{
		if($newbrightness > 100 || $newbrightness < 0)
			$newbrightness=85;
		$setbrightness = round($newbrightness *236/100);
		$command = "echo '$setbrightness' > /etc/brightness";
		exec ($command);
		$command = "echo '$setbrightness' > /sys/class/backlight/pwm-backlight.0/brightness";
		exec ($command);
		$command = "echo '$setbrightness' > /sys/class/backlight/pwm-backlight.1/brightness";
		exec ($command);
		header("Refresh:0");
	}
?>
